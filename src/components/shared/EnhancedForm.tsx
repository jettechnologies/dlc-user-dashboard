...
type UseFormHookProps<T extends z.ZodTypeAny> = {
	schema: T
	defaultValues?: DefaultValues<z.infer<T>>
	...
	return (
		<div className="relative">
			<input
				...
				required={required}
				className={twMerge(
					`block w-full rounded-lg bg-[#fbfbfb] font-poppins font-normal text-black ${sizeClasses[inputSize]} ${
						type === "password" ? "pr-12" : ""
					}`,
					className
				)}
...
export const EnhancedForm = {
	useFormHook: <T extends z.ZodTypeAny>({
		schema,
		defaultValues,
		mode = "onChange"
